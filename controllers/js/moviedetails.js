document.addEventListener("DOMContentLoaded", () => {
        ...
        const endpointM = `${BASE_URL}/movie/${movieId}?api_key=${API_KEY}&language=fr-FR&page=1`;
        const endpointS = `${BASE_URL}/serie/${serieId}?api_key=${API_KEY}&language=fr-FR&page=1`;
        ...
        fetch(endpoint)
            ...
            .then(data => {
                ...
                async function getMovieDetails(movieId) {
                    const req = await fetch(`${BASE_URL}/movie/${movieId}?api_key=${API_KEY}&language=fr-FR`);
                    return await req.json()
                }

                async function getSeriesDetails(serieId) {
                    const req = await fetch(`${BASE_URL}/serie/${serieId}?api_key=${API_KEY}&language=fr-FR`);
                    return await req.json()
                }

                async function getMovieCredits(movieId) {
                    const req = await fetch(`${BASE_URL}/movie/${movieId}/credits?api_key=${API_KEY}&language=en-US`);
                    return await req.json();
                }

                async function getSerieCredits(serieId) {
                    const req = await fetch(`${BASE_URL}/serie/${serieId}/credits?api_key=${API_KEY}&language=en-US`);
                    return await req.json();
                }
                ...
                async function displaySerieDetails() {
                    try {
                        const movie = await getMovieDetails(serieId);
                        const credits = await getMovieCredits(serieId);
                ...
                async function displaySerieReviews() {
                    try {
                        const reviews = await getMovieReviews(movieId);
                        const comList = document.getElementById("comList");
                ...
                if (URLSearchParams(window.location.search).get("movie")) {
                    displayMoviesDetails();
                    displayMovieReviews();
                } else if (URLSearchParams(window.location.search).get("serie")) {
                    displaySerieDetails();
                    displaySerieReviews();
                } else {
                    return
